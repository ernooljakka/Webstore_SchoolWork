...
const getUser = (email, password) => {
  const fetchedUser = data.users.find(user => user.email === email && user.password === password);
 ...
 */
 const getUserById = userId => {
  ...
  const objIndex = users.findIndex((user) => user._id === userId);
  if(objIndex !== -1) {
    return users[objIndex];
  }
 ...
 */
 const deleteUserById = userId => {
  ...
  const objIndex = data.users.findIndex((user) => user._id === userId);
  if(objIndex !== -1) {
      const deleted = data.users[objIndex];
      data.users.splice(objIndex, 1);
      return deleted;
  }
...
//This was done by me
 const updateUserRole = (userId, role) => {
  ...
  const objIndex = data.users.findIndex((user) => user._id === userId);
  if(objIndex !== -1) {
    data.users[objIndex].role = role;
    data.roles.splice(objIndex, 1);
    data.roles.splice(objIndex, 0, role);
      const updatedUser = data.users[objIndex];
