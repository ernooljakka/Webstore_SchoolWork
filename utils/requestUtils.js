...
const getCredentials = request => {
  ...
  const encodedAuth = request.headers.authorization;
  if (encodedAuth && encodedAuth.startsWith('Basic')) {
    const encodedData = encodedAuth.split(' ')[1];
    const buff = Buffer.from(encodedData, 'base64');
    const decodedData = buff.toString('ascii').split(':');
    return decodedData;
  } 
  else {
    return null;
